def main():
    ...
    #Student Info
    print("\nStudent Information:")
    print("Student ", '\t\t'.join(list([key for key in dict(student_info.items()).values()][0].keys())))
    for student, info in student_info.items():
        print(student, '\t', '\t\t'.join(['\t\t' + str(info[key]) if key in ("Credits_Completed","Grades") else str(info[key]) for key in info]))
